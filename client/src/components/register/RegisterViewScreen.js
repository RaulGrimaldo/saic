...
const RegisterViewScreen = (
    {   
        auth: {user},
        visibleRegisterViewScreen,
        setVisibleRegisterViewScreen,
        rowData= {roData: JSON.parse(localStorage.getItem("rowdata"))},
         register: { updated }, updateregister, restartregister
    }) => {
    ...
    const {_id, ID, JUD,fecharango } = rowData.roData;
    

    let { datospersona1, datospersona2, datospersona3, datospersona4,
            datospersona5, datospersona6, datospersona7, datospersona8} = rowData.roData;

    const {/* userprograma1,
    userprograma2, */ estrategia, Tipo, Actividad, Rubro, Subrubro, turno,  claveCCT, matricula, sectoreducativo} = rowData.roData;

    const { grupovulnerable, Solicitudes} = rowData.roData;

    const { TemasTratados, Acuerdos, observaciones, foliosasociados } = rowData.roData;
    ...
    const { actividaddgpd, actividadcdhcdmx, actividadccsjcdmx,
        ...
        actividadusec, otrasareas, } = rowData.roData;
    
    const { institucion, Calle, Numero, Alcaldia, Colonia, 
        Cuadrante, sector, coordx, coordy } = rowData.roData;

    const { Contacto, Cargo, Email, Telefono } = rowData.roData;
    const { Asistentes,  asistentesmujeres, asistenteshombres,
            asistentes014, asistentes1518, asistentes1965, asistentes65mas } = rowData.roData;

    const {capturista, fechacreacion,  validasub } = rowData.roData;
    ...
    /* if (updated) {    
        ...
        setTimeout(() => {
            setUpdateButton(false);
            setVisibleRegisterViewScreen(false);
        }, 200);    
    } */
                ...
                <div className="card-body">             
                    ...
                    >
                        ...
                        >
                            ...
                            <Panel header="VIII. EVIDENCIA FOTOGRÁFICA" key="9">
                            ...
                            <Panel header="IX. ACTIVIDAD DE CONTROL" key="8">
                                <div className="row custom-collapse-content" >
                                
                                <p>{capturista +  + ' ' + fechacreacion}</p>
                                <p>{validasub}</p>
                                </div>          
                            </Panel>
                        </Collapse>,                                     
...
export default connect(mapStateToProps, { updateregister, restartregister })(RegisterViewScreen);
